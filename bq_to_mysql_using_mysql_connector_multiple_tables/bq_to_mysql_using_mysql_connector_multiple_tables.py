...
DAG_Name = 'bq_to_mysql_using_mysql_connector_multiple_tables' 
...
cloudsql_project = common_args['cloudsql_project']
...
def mysql_conn(mysql_host, mysql_user, mysql_password, mysql_database, file_name, mysql_table_name, insert_into_script):
    ...
    if conn.is_connected():
            ...
            for row in csv_data2.values.tolist():
                print(row)
                cursor.execute('INSERT INTO {}(emp_id, Name, emp_add ) VALUES("%s", "%s", "%s");'.format(mysql_table_name), 
                      row)
...
for i in parsed_master_yaml_file.keys():
    ...
    gcs_to_mysql = PythonOperator(
            task_id=f'gcs_to_mysql{bq_table}',
            provide_context=True,
    ...
    #Task dependancy
    START_TASK >> BIGQUERY_TO_GCS >> STORAGE_LIST_FILES >> FILES_CONCAT >>  EXECUTE_MERGE_FILES >> gcs_to_mysql >> END_TASK